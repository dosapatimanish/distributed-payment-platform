...
- [`backend-documents/end-to-end-flow.md`](backend-documents/end-to-end-flow.md) — the complete flow from creating a wallet to a recorded transaction: block diagram, UML sequence diagrams (happy path, merchant charge, compensation), the saga state machine, the Idempotency-Key map, what's recorded where, and a runnable curl walkthrough.
- [`backend-documents/identifier-scheme.md`](backend-documents/identifier-scheme.md) — the bank-style identifier scheme (implemented): `userId` → 10-digit CIF, `walletId` → 12-char account number (`[currency 2][CIF prefix 5][seq 5]`), `transactionId` → 16-digit id (`[src-currency 2][YYYYMMDD 8][daily seq 6]`), plus a `currency` reference table (wallet-service, `GET /api/v1/currencies`) and a `unit_master` bank business-date rolled by a scheduler.
