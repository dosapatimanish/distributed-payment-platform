...
1. [WalletController.captureReservation](../../backend/wallet-service/src/main/java/com/paymentplatform/wallet/web/WalletController.java) — `@RequestHeader("Idempotency-Key")` (required) + `@PathVariable reservationId`, no request DTO.
2. [IdempotencyGuard.runIdempotent](../../backend/wallet-service/src/main/java/com/paymentplatform/wallet/idempotency/IdempotencyGuard.java) — a completed key short-circuits to the cached `WalletResponse`, skipping every step below.
3. [WalletService.captureReservation](../../backend/wallet-service/src/main/java/com/paymentplatform/wallet/service/WalletService.java):
...
5. [WalletRepository](../../backend/wallet-service/src/main/java/com/paymentplatform/wallet/repository/WalletRepository.java) — via the reused `debit()` call, same as [03-debit-wallet.md](03-debit-wallet.md).
6. `WalletResponse.from(debitedWallet)` — note this returns the wallet, not a `ReservationResponse`; the caller already has the reservation details from the reserve call. On success, `IdempotencyGuard` caches this response; on failure, it releases the key.
...
- **`Idempotency-Key` complements, doesn't replace, the state machine**: the state check alone already stops a retried capture from double-debiting (the second attempt sees `CAPTURED`, not `HELD`, and errors) — but that's an *error*, not a clean replay. The key turns a legitimate retry into an identical success response instead of a spurious 409.
