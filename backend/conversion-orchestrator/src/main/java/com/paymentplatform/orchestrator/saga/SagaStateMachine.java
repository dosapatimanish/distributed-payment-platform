 ...
 * design doc's full (Merchant-Payment-inclusive) state table.
 */
public final class SagaStateMachine {
    ...
    static {
        ...
        VALID_TRANSITIONS.put(DEST_CREDITED, EnumSet.of(COMPLETED));
        VALID_TRANSITIONS.put(DEBIT_FAILED, EnumSet.of(COMPENSATING));
        VALID_TRANSITIONS.put(CREDIT_FAILED, EnumSet.of(COMPENSATING));
        // COMPENSATING forks: DEBIT_FAILED's path has nothing to reverse (skips straight to
        // LOCK_RELEASED); CREDIT_FAILED's path reverses the debit first (SOURCE_CREDITED_BACK).
        VALID_TRANSITIONS.put(COMPENSATING, EnumSet.of(SOURCE_CREDITED_BACK, LOCK_RELEASED));
