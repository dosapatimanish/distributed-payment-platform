...
import com.paymentplatform.ledger.idempotency.IdempotencyKeyInProgressException;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(InvalidLedgerEntriesException.class)
    public ResponseEntity<ErrorResponse> handleInvalidEntries(InvalidLedgerEntriesException ex, HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, "INVALID_LEDGER_ENTRIES", ex.getMessage(), req);
    }
    ...
    @ExceptionHandler({LedgerConflictException.class, DataIntegrityViolationException.class})
    public ResponseEntity<ErrorResponse> handleConflict(Exception ex, HttpServletRequest req) {
        String code = ex instanceof LedgerConflictException ? "LEDGER_CONFLICT" : "DATA_CONFLICT";
