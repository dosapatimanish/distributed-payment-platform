package com.paymentplatform.ledger.idempotency;
...
/**
 * Redis-backed safe-retry guard for write endpoints (design doc §6.2.3) - identical mechanics as
 * the other services' IdempotencyGuard (see idempotency.md for the full reasoning). Deliberate
 * independent copy, not a shared library - the fifth instance of this same pattern, one per
 * service.
 */
@Component
public class IdempotencyGuard {
    ...
    public IdempotencyGuard(StringRedisTemplate redisTemplate, ObjectMapper objectMapper,
                             @Value("${ledger.idempotency.ttl-hours}") long ttlHours,
                             @Value("${ledger.idempotency.key-prefix}") String keyPrefix) {
