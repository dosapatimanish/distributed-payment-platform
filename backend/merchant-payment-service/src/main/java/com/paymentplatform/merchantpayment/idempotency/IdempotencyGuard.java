 ...
 * and the same only-cache-success simplification as the other three services' IdempotencyGuard
 * (see idempotency.md for the full reasoning). Deliberate independent copy, not a shared
 * library - the fourth instance of this same pattern, one per service.
 *
 * Note: the design doc's own REST contract table (§6.4) doesn't call out an Idempotency-Key
 * header for {@code POST /merchant-payments} specifically (it lists "Idempotent on
 * transaction_id UNIQUE" as that endpoint's concurrency control) - this service adds one anyway,
 * same value-add reasoning as fx-rate-service's `lockRate` (see its implementation notes): the
 * unique constraint alone stops a *duplicate* payment, but turns a legitimate retry into an
 * error, not a replay. The header fixes that.
 */
@Component
public class IdempotencyGuard {
    ...
    public IdempotencyGuard(StringRedisTemplate redisTemplate, ObjectMapper objectMapper,
                             @Value("${merchantpayment.idempotency.ttl-hours}") long ttlHours,
                             @Value("${merchantpayment.idempotency.key-prefix}") String keyPrefix) {
